                ...
                <div class="row Item">
                    ...
                    <div class="col-4">
                        <div class="shopping-cart-quantity d-flex justify-content-between align-items-center h-100 border-bottom pb-2 pt-3">
                            <input class="quantity-input botonCambiar" type="number" value="${cantidad}" min="1" max="10">
                            <button class="btn btn-success botonAumentar" id="btn-add" type="button" marcador=${articulo[0].id}>+</button>
                            <button class="btn btn-danger botonDisminuir" id="btn-add" type="button" marcador=${articulo[0].id}>-</button>
                            <button class="btn btn-danger botonBorrar" type="button" marcador=${articulo[0].id}>X</button>
                        </div>
                    </div>
                </div>`);
            });
        $('.botonAumentar').click(aumentarCarrito);
        $('.botonDisminuir').click(reducirCarrito);
...
function reducirCarrito(){
    ...
    carrito = carrito.reduce((total, itemId) => {
        return itemId === marcador ? total : total.concat(itemId);
    }, []);
    console.log(carrito);
    calcularTotal();
    renderCarrito();
